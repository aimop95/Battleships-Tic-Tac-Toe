...
(defun play (turn)
	(cond
		...
		((equal turn 'C)
			...
			(let ((compList (compTurn board) )) ;;See compTurn comment below
			;(setf compList (compTurn board))
	...
	(defun compTurn (board) ;;Currently, the computer picks a random empty spot on the grid and plays there.
		(setf row (random 3))
		(setf col (random 3))
		(if (equal (nth col (nth row board)) '-)
			(return-from compTurn (list row col))
			(compTurn board)
			)
		)
